...
local function ExchangeSuccess()
	TriggerServerEvent('qb-cryptostick:server:ExchangeSuccess', math.random(1, 10))
end
...
local function ExchangeFail()
	...
	if RemoveChance == LosingNumber then
		TriggerServerEvent('qb-cryptostick:server:ExchangeFail')
		TriggerServerEvent('qb-cryptostick:server:SyncReboot')
	end
end
	...
	CreateThread(function()
		while Crypto.Exchange.RebootInfo.state do
			if (Crypto.Exchange.RebootInfo.percentage + 1) <= 100 then
				...
				TriggerServerEvent('qb-cryptostick:server:Rebooting', true, Crypto.Exchange.RebootInfo.percentage)
			else
				Crypto.Exchange.RebootInfo.percentage = 0
				Crypto.Exchange.RebootInfo.state = false
				TriggerServerEvent('qb-cryptostick:server:Rebooting', false, 0)
			end
			Wait(1200)
		end
	end)
				...
				if dist < 1.5 then
					if not Crypto.Exchange.RebootInfo.state then
						...
						if IsControlJustPressed(0, 38) then
							QBCore.Functions.TriggerCallback('qb-cryptostick:server:HasSticky', function(HasItem)
								if HasItem then
...
RegisterNetEvent('qb-cryptostick:client:SyncReboot', function()
	Crypto.Exchange.RebootInfo.state = true
	SystemCrashCooldown()
end)


RegisterNetEvent('qb-cryptostick:client:GetRebootState', function(RebootInfo)
